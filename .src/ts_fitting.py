...
def flat_peak(ax5, xml):
    ...
    for i, wavelength_sweep in enumerate(root.iter('WavelengthSweep')):
        if i != 6:
            ...
            params = model.make_params(I_0=0.5, n_eff=2.6)  # 변수들을 값을 고정하거나 초기화 함
            # Fit the model to the data
            result = model.fit(wavelength_data['measured_transmission'], wavelength=wavelength)
            ax5.plot(wavelength_data['wavelength'], result.best_fit, linestyle='-', lw=1, color='r',
...
#     return I_0 * (sin(pi / wavelength * (40 * n_eff + l * del_n_eff)) ** 2)
#
#
# # Approach the path
